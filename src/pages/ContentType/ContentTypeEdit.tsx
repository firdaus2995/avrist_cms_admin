          ...
          <div key={idx} className='py-2 px-10 flex flex-row justify-between m-4 bg-light-purple-2 rounded-lg hover:border-2 font-medium'>
            ...
            <div className='w-1/4 flex flex-row gap-5 items-center justify-center'>
              <img role='button' onClick={() => {
                setEditedIndex(idx);
                const edited = listAttributes?.filter(value => value.code === getFieldId(val.fieldType))
    ...
    return (
      <Modal open={isOpenModalEditAttribute} toggle={() => null} title="" width={840} height={480}>
        <div className='flex flex-col'>
          ...
          <div className='flex flex-col mx-10'>
            <div className='p-4 capitalize font-bold border-b-2 mb-4'>{getType(openedAttribute?.code)}</div>
            <div className='flex flex-col w-1/2'>
              <InputText 
                labelTitle='Name' 
                labelRequired

...
const LeadsGeneratorConditions = () => {
  ...
  return (
    <TitleCard
      ...
      topMargin="mt-2"
      TopSideButtons={
        <div className={styleButton({variants: "primary"})} onClick={() => {}}>
          Add Condition
        </div>
      }
    >
        ...
        <div className="flex flex-col gap-y-2">
          ...
          <div className="bg-table rounded-xl relative">
            {arrData().map((item: IListItem, idx: number) => (
              <div key={idx} className={`${idx+1 === arrData().length ? "" : "border-b border-light-grey"} flex gap-x-3 px-4 py-2`}>
                ...
                <div className="flex w-[5%] gap-x-1">
                  ...
                  >x</div>
                  <div className={`!min-w-[24px] !h-[24px] !rounded-lg ${styleButton({variants: "error"})}`}>x</div>
                </div> 
              </div>
            ))}
          </div>          

...
export default function LeadsGeneratorConditions() {
  ...
  // RTK GET DATA
  const fetchQuery = useGetConditionListQuery({
    pageIndex,
    limit: pageLimit,
    direction,
    sortBy,
  ...
  // RTK DELETE
  const [deleteCondition, { isLoading: deletePageLoading }] = useDeleteConditionMutation();

  useEffect(() => {
    if (data) {
      setListData(data?.conditionList?.conditions);
      setTotal(data?.conditionList?.total);
    }
  }, [data]);
  ...
  const COLUMNS = [
    ...
    {
      header: () => <span className="text-[14px]">Condition Title</span>,
      accessorKey: 'title',
      enableSorting: true,
      cell: (info: any) => (
    ...
    },
    {
      ...
      accessorKey: 'resultTemplateName',
      enableSorting: false,
      cell: (info: any) => (
        <div className="flex justify-start items-start">
    ...
    {
      header: () => <span className="text-[14px]">Condition</span>,
      ...
      cell: (info: any) => (
        <div className="flex justify-start items-start">
          <p className="text-[14px] truncate">
            {info.getValue() && info.getValue() !== '' && info.getValue() !== null
              ? `${info?.row?.original?.totalQuestion} question and ${info?.row?.original?.totalAnswer} answers selected`
              : '-'}
          </p>
        </div>
      ),
      ...
      cell: (info: any) => {
        return (
          <div className="flex gap-3">
            <Link to={`/conditions/${info.getValue()}`}>
